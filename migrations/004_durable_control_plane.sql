...
CREATE TABLE IF NOT EXISTS netengine_control.evidence(evidence_id text PRIMARY KEY, run_id text NOT NULL REFERENCES netengine_control.execution_runs, operation_id text, created_at timestamptz NOT NULL, payload jsonb NOT NULL);
CREATE TABLE IF NOT EXISTS netengine_control.checkpoints(checkpoint_id text PRIMARY KEY, run_id text NOT NULL REFERENCES netengine_control.execution_runs, previous_checkpoint_id text REFERENCES netengine_control.checkpoints, schema_version integer NOT NULL, plan_fingerprint text NOT NULL, created_at timestamptz NOT NULL, payload jsonb NOT NULL);
CREATE TABLE IF NOT EXISTS netengine_control.leases(lock_key text PRIMARY KEY, owner text NOT NULL, acquired_at timestamptz NOT NULL, expires_at timestamptz NOT NULL, fencing_token bigint NOT NULL, released_at timestamptz, payload jsonb NOT NULL);
