...
def execute(executor, plan, **kwargs):
    return executor.execute(plan, facts=offline_demo_facts(plan, datetime.now(UTC)), **kwargs)
...
def test_apply_observe_evidence_and_idempotent_reexecution():
    ...
    report = run(execute(executor, executable))
    ...
    resumed = run(execute(executor, executable, run_id=report.run.id))
...
def test_runtime_never_invents_sovereign_facts_from_the_plan():
    _, _, plan = admitted_plan()
    registry = setup()
    executable = compile_plan(plan, registry)
    with pytest.raises(ValueError, match="runtime facts are required"):
        run(RuntimeExecutor(registry, InMemoryExecutionRepository()).execute(executable))
...
def test_dry_run_validates_without_mutation_and_reports_prediction():
    ...
    report = run(
        execute(
            RuntimeExecutor(registry, InMemoryExecutionRepository()),
            compile_plan(plan, registry),
            dry_run=True,
        )
    )
...
def test_validation_observation_and_retry_failure_semantics():
    ...
    second = run(execute(RuntimeExecutor(registry, InMemoryExecutionRepository()), executable))
...
def test_partial_failure_compensates_completed_operations_in_reverse_order():
    ...
    report = run(execute(RuntimeExecutor(registry, InMemoryExecutionRepository()), executable))
    compensated = [item for item in report.run.operations.values() if item.state == "compensated"]
    assert compensated
    assert provider.state[executable.operations[0].target] == executable.operations[0].prior_value
...
def test_interruption_after_apply_resumes_by_observation_without_duplicate():
    ...
    report = run(execute(RuntimeExecutor(registry, repository), executable, run_id=run_record.id))
    assert report.status == "succeeded"
    ...
    with pytest.raises(ValueError):
        run(execute(RuntimeExecutor(registry, repository), changed, run_id=run_record.id))
...
def test_dry_run_failure_never_compensates_simulated_operations():
    ...
    report = run(
        execute(RuntimeExecutor(registry, InMemoryExecutionRepository()), executable, dry_run=True)
    )
